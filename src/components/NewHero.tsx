...
const NewHero = () => {
  ...
  return <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
          ...
          <div className="backdrop-blur-md border border-white/20 p-6 shadow-2xl px-[18px] rounded-3xl transition-all duration-300 group bg-black/40 hover:bg-black/60 py-[15px]">
            <form onSubmit={handleSubmit} className="space-y-1">
              ...
              <div className="flex justify-between items-center">
                <Button type="button" variant="ghost" size="icon" className="w-10 h-10 rounded-full text-white transition-all bg-transparent hover:bg-white/20 hover:text-white text-base">
                  <Plus size={20} />
                </Button>

                <div className="flex gap-2">
                  <Button type="button" variant="ghost" size="icon" className="w-10 h-10 rounded-full text-white transition-all bg-transparent hover:bg-white/20 hover:text-white">
                    <Mic size={20} />
                  </Button>
                  
                  <Button type="submit" variant="ghost" size="icon" className="w-10 h-10 rounded-full text-white transition-all bg-transparent hover:bg-white/20 hover:text-white">
                    <ArrowUp size={20} />
                  </Button>
                </div>
              </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-10">
        <Button onClick={scrollToNextSection} variant="ghost" size="icon" className="w-10 h-10 text-white transition-all animate-bounce-slow rounded-full bg-transparent hover:bg-white/10">
          <ChevronDown size={20} />
        </Button>
      </div>
    </section>;
