...
const WhatsAppAssistant = () => {
  ...
  return <section id="whatsapp-assistant" className="py-20 bg-gray-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {categories.map(category => <button key={category.id} onClick={() => setActiveCategory(category.id)} className={`w-full p-6 rounded-xl border-2 transition-all duration-300 text-left ${activeCategory === category.id ? 'border-green-500 bg-green-50 shadow-lg' : 'border-gray-200 bg-white hover:border-green-300 hover:shadow-md'}`}>
                  <div className="flex items-start gap-4">
                    <div className="text-3xl">{category.icon}</div>
                    <div>
                      <h4 className={`text-xl font-semibold mb-2 ${activeCategory === category.id ? 'text-green-600' : 'text-gray-900'}`}>
                        {category.title}
                      </h4>
        ...
        <div className="mt-16">
          <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {benefits.map((benefit, index) => <div key={index} className="text-left">
