...
const categorizeServices = (services: Gallery4Item[]) => {
  ...
  const terceirizacaoIds = ["terceirizacao-contabil", "terceirizacao-fiscal", "terceirizacao-dp", "terceirizacao-paralegal", "bpo-financeiro"];
  const consultoriaIds = ["planejamento-tributario", "assessoria-tributaria", "consultoria-tecnica-ia", "estruturacao-holdings", "cursos-treinamentos"];
  const servicosDigitaisIds = ["certificados-digitais", "radar-receita", "registro-marca", "atendimento-ia-whatsapp", "endereco-fiscal", "recrutamento-ia", "marketing-digital", "sites-contabeis"];
  return {
    terceirizacao: services.filter(service => terceirizacaoIds.includes(service.id)),
    consultoria: services.filter(service => consultoriaIds.includes(service.id)),
    servicosDigitais: services.filter(service => servicosDigitaisIds.includes(service.id))
  };
};
...
const CategoryCarousel = ({
  ...
  return <div className="mb-16 last:mb-0">
      ...
      <div className="w-full relative">
        ...
        }} disabled={!canScrollPrev} className="absolute left-4 top-[13.5rem] -translate-y-1/2 z-20 h-12 w-12 bg-white/90 hover:bg-white shadow-lg rounded-full disabled:opacity-50 disabled:pointer-events-none transition-all duration-200">
        ...
        }} disabled={!canScrollNext} className="absolute right-4 top-[13.5rem] -translate-y-1/2 z-20 h-12 w-12 bg-white/90 hover:bg-white shadow-lg rounded-full disabled:opacity-50 disabled:pointer-events-none transition-all duration-200">
            <ArrowRight className="size-5" />
          </Button>

          <CarouselContent className="ml-0 2xl:ml-[max(8rem,calc(50vw-700px))] 2xl:mr-[max(0rem,calc(50vw-700px))]">
            {items.map(item => <CarouselItem key={item.id} className="max-w-[320px] pl-[20px] lg:max-w-[360px]">
                <a href={item.href} className="group rounded-xl">
                  <div className="group relative h-full min-h-[27rem] max-w-full overflow-hidden rounded-xl md:aspect-[5/4] lg:aspect-[16/9] transition-all duration-300 flex flex-col justify-end">
                    <img src={item.image} alt={item.title} className="absolute inset-0 h-full w-full object-cover object-center transition-opacity duration-300" />
        ...
        {items.length > 1 && <div className="mt-8 flex justify-center gap-2">
            {Array.from({
          length: totalDots
        }).map((_, index) => <button key={index} className={`h-2 w-2 rounded-full transition-colors ${activeDotIndex === index ? "bg-primary" : "bg-primary/20"}`} onClick={() => carouselApi?.scrollTo(index)} aria-label={`Ir para slide ${index + 1}`} />)}
          </div>}
      </div>
    </div>;
};
...
const CategorizedGallery = ({
  ...
  return <section className="pb-32 pt-20 bg-white">
      ...
      {/* Terceirização */}
      <CategoryCarousel title="Terceirização" items={categorizedServices.terceirizacao} />

      {/* Consultoria & Assessoria */}
      <CategoryCarousel title="Consultoria & Assessoria" items={categorizedServices.consultoria} />

      {/* Serviços Digitais */}
      <CategoryCarousel title="Serviços Digitais" items={categorizedServices.servicosDigitais} />
    </section>;
};
...
const Gallery4 = ({
  ...
  const totalDots = Math.min(items.length, 15);
  const activeDotIndex = Math.round(scrollProgress * (totalDots - 1));
  return <section className="bg-slate-100 pb-32 pt-20">
      <div className="container mx-auto">
